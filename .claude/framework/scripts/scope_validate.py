#!/usr/bin/env python3
"""
Scope Dependency Validator for HaroFramework Documentation

This script enforces the 2-scope architecture rule:
✅ ALLOWED:    Game → Framework (games can use framework)
❌ FORBIDDEN:  Framework → Game (framework CANNOT reference any game)

Usage:
    python scope_validate.py [--fix] [--verbose]
...
class Document:
    ...
    path: Path
    scope: str  # "framework" or "game"
    title: str
    references: List[str]
    parent_documents: List[str]

    def __str__(self):
        return f"{self.scope}/{self.path.relative_to(self.path.parents[4])}"


@dataclass
class Violation:
    """Represents a scope dependency violation"""
    source_doc: Document
    target_path: str
    target_scope: str
    ...
    def __str__(self):
        return (
            f"[X] VIOLATION: {self.source_doc.scope} document references {self.target_scope}\n"
...
class ScopeValidator:
    """Validates scope dependencies in documentation"""
    ...
    def load_documents(self):
        ...
        for md_file in claude_dir.rglob('*.md'):
            ...
            try:
                ...
                # Determine scope from path or metadata (support both 'scope' and legacy 'layer')
                scope = metadata.get('scope', metadata.get('layer', 'unknown'))
                ...
                self.log(f"  [+] Loaded: {doc.title} (scope: {scope})")
    ...
    def determine_scope_from_path(self, ref_path: str, source_doc: Document) -> str:
        """Determine the scope of a referenced document from its path"""
        ...
        # Check if path contains scope indicators
        ...
        try:
            ...
            # Check if path exists and determine scope
        ...
        # Default: assume same scope if we can't determine
        return source_doc.scope

    def validate_dependencies(self):
        """Validate that Framework documents don't reference Game documents"""
        print("[*] Validating scope dependencies...\n")

        for doc in self.documents:
            if doc.scope != 'framework':
            ...
            for ref in doc.references:
                target_scope = self.determine_scope_from_path(ref, doc)
            ...
            for parent in doc.parent_documents:
                target_scope = self.determine_scope_from_path(parent, doc)
    ...
    def report_violations(self) -> int:
        ...
        if not self.violations:
            ...
            print("    All scope dependencies are correct!")
            print("    Framework documents do not reference Game scope.")
            return 0

        print("[X] VALIDATION FAILED")
        print(f"    Found {len(self.violations)} scope dependency violation(s):\n")
    ...
    def run(self) -> int:
        ...
        print("=" * 70)
        print("Scope Dependency Validator")
        print("=" * 70)
        print()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Validate scope dependencies in HaroFramework documentation'
    )
    parser.add_argument(
        '--verbose', '-v',
        action='store_true',
    ...
    # Run validation
    validator = ScopeValidator(root_path, verbose=args.verbose)
